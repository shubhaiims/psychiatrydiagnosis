...
- `backend/game-config.mjs`: editable app name, mode rules, cue counts, and how-to-play copy.
...
- `data/static-cases.json`: generated answer-containing reference data blocked by the backend server.
...
- `GET /api/cases/today?mode=easy|difficult&date=YYYY-MM-DD`: public 5-question round payload, one starting cue per case, and no answers.
...
- `data/static-cases.json` is also blocked by `server.mjs` because it includes answer IDs.
- For production, keep paid and answer-containing data in a private server, private repo, or database.
- Frontend files should not be used for game-rule or content editing.

## Common Edits

- Rename the app: edit `appName` in `backend/game-config.mjs`.
- Change mode labels, round size, difficulty selection, cue counts, guess limits, and how-to-play copy: edit `backend/game-config.mjs`.
...
Use `DEPLOYMENT.md` for hosting guidance. Deploy this project as one Node web service so the backend and frontend stay together.
