...
4. The current 5-question round is loaded from `/api/cases/today`.
5. Guesses are validated by `/api/guess`.
6. Wrong guesses receive exactly one new cue from the backend until the six-cue limit.
7. Solved or missed questions unlock the Next Question control until all five questions are complete.
8. Plus content is requested from `/api/premium/diagnosis/:id` after token entry.
