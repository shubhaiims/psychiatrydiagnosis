...
- Game mode buttons: `easyMode`, `classicMode`, `toughMode`
...
4. Easy Mode starts with 4 clues and Classic Mode starts with 1 clue.
5. In Classic Mode and Easy Mode, wrong guesses request the next clue from `/api/guess`.
6. Tough Mode starts with 5 clues; the player can click `Reveal next clue`, which calls `/api/reveal`.
7. After the case is complete, Plus content is requested from `/api/premium/diagnosis/:id`.
8. If the backend is unavailable, the frontend falls back to `data/static-cases.json` for demo use.
...
- Change Easy, Classic, or Tough behavior: edit `MODE_CONFIG`, `submitGuess`, `revealNextClue`, and `maxGuessesForMode` in `app.js`.
